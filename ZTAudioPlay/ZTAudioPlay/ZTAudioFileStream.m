...
@implementation ZTAudioFileStream
...
static void ZTAudioFileStreamPropertyListenerCallBack(void *inClientData,
                                               AudioFileStreamID inAudioFileStreamID,
                                               AudioFileStreamPropertyID inPropertyID,
                                               UInt32 *ioFlags)
{
    ZTAudioFileStream *audioFileStream = (__bridge ZTAudioFileStream *)inClientData;
    [audioFileStream handleAudioFileStreamProperty:inPropertyID];
}

static void ZTAudioFileStreamPacketsCallBack(void *inClientData,
...
- (BOOL)_openAudioFileStreamWithFileTypeHint:(AudioFileTypeID)fileTypeHint error:(NSError *__autoreleasing *)error
{
    OSStatus status = AudioFileStreamOpen((__bridge void *)self,
                                          ZTAudioFileStreamPropertyListenerCallBack,
                                          ZTAudioFileStreamPacketsCallBack,
                                          fileTypeHint,
                                          &_audioFileStreamID);
                  ...
                  packetDescriptions:(AudioStreamPacketDescription *)packetDescriptioins
{
    ...
    for (int i = 0; i < numberOfPackets; ++i)
    {
        ...
        ZTAudioParsedData *parsedData = [ZTAudioParsedData parsedAudioDataWithBytes:packets + packetOffset
