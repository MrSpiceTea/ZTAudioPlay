...
const int ZTAudioQueueBufferCount = 2;

@interface ZTAudioQueueBuffer : NSObject
@property (nonatomic,assign) AudioQueueBufferRef buffer;
@end
@implementation ZTAudioQueueBuffer
@end

@interface ZTAudioOutputQueue()
{
@private
...
@implementation ZTAudioOutputQueue
...
- (void)_createAudioOutputQueue:(NSData *)magicCookie
{
    ...
    if (_buffers.count == 0)
    {
        for (int i = 0; i < ZTAudioQueueBufferCount; ++i)
        {
            ...
            ZTAudioQueueBuffer *bufferObj = [[ZTAudioQueueBuffer alloc] init];
...
- (BOOL)playData:(NSData *)data packetCount:(UInt32)packetCount packetDescriptions:(AudioStreamPacketDescription *)packetDescriptions isEof:(BOOL)isEof
{
    ...
    ZTAudioQueueBuffer *bufferObj = [_reusableBuffers firstObject];
    ...
    if (!bufferObj)
    {
        ...
        if (status == noErr)
        {
            bufferObj = [[ZTAudioQueueBuffer alloc] init];
            bufferObj.buffer = buffer;
        }
        else
        {
...
{
    ZTAudioOutputQueue *audioOutputQueue = (__bridge ZTAudioOutputQueue *)inClientData;
...
{
    ZTAudioOutputQueue *audioQueue = (__bridge ZTAudioOutputQueue *)inUserData;
